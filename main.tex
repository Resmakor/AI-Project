...
Skrypt został podzielony na kilka innych, w zależności od celu.
...
W konstruktorze zdefiniowana zostaje struktura modelu, tzn. warstwy liniowe `nn.Linear' i ich rozmiary.
...
Występują również dwie zagnieżdżone pętle, które iterują po liczbie warstw z zakresu 3 - 10 oraz po współczynniku uczenia.
...
Na końcu rysowany jest wykres zależności poprawności klasyfikacji od liczby warstw w sieci i współczynniku uczenia.
\subsection{Skrypt dla zależności poprawności klasyfikacji od liczby warstw oraz współczynnika uczenia [6]}
\begin{verbatim}
class Model(nn.Module):
...
W kodzie pojawiła się również lista `activations', która zawiera funkcję aktywacji takie jak ReLU, tangens hiperboliczny oraz funkcję sigmoidalną.
...
Na końcu rysowany jest wykres zależności poprawności klasyfikacji od użytej funkcji przejścia.
...
Hipoteza zostanie zweryfikowana w drugim eksperymencie.
...
Pozostałe parametry pozostały bez zmian.
\begin{figure}[H]
    \centering
    ...
    \label{fig:k1k2_2}
\end{figure}
Sieć uczyła się zdecydowanie dłużej. Okazało się jednak, że hipoteza z eksperymentu pierwszego jest prawdziwa.
Wykres jest płaski również dla większej liczby neuronów.
Minimalnie wzrosła też maksymalna poprawność klasyfikacji z $87.17$ do $92.30$ dla K1 = 90 oraz K2 = 8.
\subsection{Eksperyment 3}
Eksperyment trzeci, będzie polegał na szukaniu takich metaparametrów, aby dodać nieco głębi do wykresu.
Zmienione zostaną teraz również inne parametry.
Tym razem współczynnik uczenia będzie wynosić $0.001$, a maksymalna liczba epok $1000$.
Warstwa pierwsza i druga zawierają liczbę neuronów z zakresu 10 - 400 z krokiem 10.
\begin{figure}[H]
    \centering
    ...
    \label{fig:k1k2_3}
\end{figure}
Jak widać na powyższym rysunku, sieć zachowuje się chaotycznie utrzymując jednak dosyć wysoki poziom poprawności klasyfikacji.
\subsection{Eksperyment 4}
Tym razem eksperyment będzie polegał na zmianie liczby warstw oraz współczynniku uczenia sieci.
Wykorzystany został do tego różniący się od poprzedniego skrypt.
Liczba neuronów wynosi 6, jeżeli indeks warstwy jest parzysty oraz 4 w przeciwnym wypadku.
Liczba epok to 100, a maksymalna liczba warstw to 10.
...
Być może liczba epok wynosząca 100 jest zbyt mała.
Kolejny eksperyment to zweryfikuje.

\subsection{Eksperyment 5}
Eksperyment będzie analogiczny do poprzedniego, z tym że liczba epok wzrośnie do 3000.

Przy liczbie warstw większej od 5 oraz współczynniku uczenia mniejszym od $10^-6$ sieć zachowuje się niestabilnie.
Jest to podobne zjawisko do tego w poprzednim eksperymencie.
Liczba epok nie ma tu zbyt wielkiego znaczenia.
PK dla 9 warstw oraz LR 0.1 84 procent max
...
Eksperyment ten sprawdzi co stanie się, gdy zwiększymy liczbę neuronów dla `K1' = 1500 oraz `K2' = 1000.
Liczba epok zostanie przywrócona na 100.
...
Okazało się, że wynik jest podobny.
Przy liczbie warstw większej od 5 oraz współczynniku uczenia mniejszym od $10^-6$ sieć zachowuje się niestabilnie.
Dla przeciwnych wartości dominuje identyczna wartość poprawności klasyfikacji tzn. 79.
Dla LR: $0.001$, L warstw 4 PK 87


\subsection{Eksperyment 7}
Tym razem zostanie zbadany wpływ użytej funkcji przejścia.
Dotychczas do warstwy pierwszej oraz ukrytych była używana funkcja ReLU.
Dla porównania wykorzystane zostaną funkcje aktywacji takie jak sigmoidalna oraz tangens hiperboliczny.
Eksperyment zostanie przeprowadzony dla suboptymalnych wartości z eksperymentu pierwszego: `K1' = 6, `K2' = 4, współczynnik uczenia wynosi $0.1$ natomiast liczba epok to 100.
...
\begin{figure}[H]
    ...
    \label{fig:activ_1}
\end{figure}
Z użyciem tangensa hiperbolicznego jako funkcji przejścia, sieć miała najwyższą poprawność klasyfikacji wynoszącą 79,48.
Różnica między pozostałymi funkcjami nie była jednak taka duża ($2.56$).

Skrypt został uruchomiony ponownie, dla tych samych parametrów.
Tym razem wynik był inny.
\begin{figure}[H]
    \centering
    ...
    \label{fig:activ_2}
\end{figure}
Większą poprawnością klasyfikacji charakteryzowała się użyta funkcja sigmoidalna (82), natomiast tangens hiperboliczny 79.48 i ReLU 76.92.
Powodem zmiany poprzedniego wyniku może być początkowe ustalenie wag, które odbywa się w sposób losowy.
Oznacza to, że funkcja aktywacji w tym przypadku nie odgrywa tak znaczącej roli.
\subsection{Eksperyment 8}
Być może wynik zmieni się, gdy użyta zostanie większa liczba warstw oraz większa liczba neuronów w sieci.
Teraz liczba warstw będzie wynosić 5, liczba neuronów dla każdej warstwy 100, współczynnik uczenia $0.1$, a liczba epok 1000.
...
Funkcja ReLU oraz sigmoidalna uzyskały ten sam poziom 79.48.

Gdy skrypt został uruchomiony ponownie tangens hiperboliczny znowu uzyskał najniższą poprawność klasyfikacji.
Oznacza to, że dla takich parametrów najlepiej użyć funkcji ReLU lub sigmoidalnej.
\section{Wnioski}
...
Parametry można było dobrać w taki sposób, że liczba neuronów w sieci nie miała zbyt dużego wpływu na poprawność klasyfikacji.
Miała jednak ogromny wpływ jeżeli chodzi o szybkość nauki, gdyż w eksperymencie drugim na koniec działania skryptu czekałem kilkanaście razy dłużej względem pierwszego.
