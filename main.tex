...
Sieć neuronowa to jedno z najbardziej fascynujących paradygmatów programowania, jakie kiedykolwiek powstały.
W tradycyjnym podejściu do programowania komputerowi nakazujemy, co ma zrobić, dzieląc duże problemy na wiele mniejszych, precyzyjnie określonych zadań, które komputer może łatwo zrealizować.
...
Są one szeroko wdrażane przez takie firmy jak Google, Microsoft i Facebook. [1]


\subsection{Historia sieci neuronowych}


Historia sztucznych neuronów sięga lat 40. XX wieku, kiedy Warren McCulloch (neurobiolog) i Walter Pitts (logik) zaproponowali model pierwszego sztucznego neuronu.


Zainspirowany publikacją Warrena McCullocha i Waltera Pittsa, Frank Rosenblatt pracował w latach 50. nad perceptronem, czyli pojedynczą warstwą neuronów zdolną do klasyfikacji obrazów o wielkości kilkuset pikseli.
...
Spowodowało to pierwszą "zimę sieci neuronowych" do połowy lat 80. [3]

Paul Werbos, w swojej pracy doktorskiej z 1974 roku, jako pierwszy zaproponował wykorzystanie propagacji wstecznej do optymalizacji sieci neuronowych.
Jednak, jako że byliśmy w pierwszej zimie sieci neuronowych, jego praca przeszła niezauważona przez środowisko naukowe.
Dopiero później, za sprawą pracy Rumelharta et al. (1986), propagacja wsteczna została spopularyzowana jako technika treningowa dla sieci neuronowych.
...
W miarę jak sieci neuronowe stawały się coraz głębsze (z większą liczbą warstw), wsteczna propagacja stawała się wolniejsza (problem zanikającego gradientu). [3]

Na początku lat 2010, wiele badań zalecało użycie określonej funkcji aktywacji (ReLu) w neuronach do przekazywania informacji.
To, wraz z lepszymi algorytmami optymalizacji (takimi jak ADAM), pomogło w efektywnym trenowaniu głębszych sieci neuronowych. [3]

W ostatnich latach dokonano znaczącego postępu w dziedzinie sztucznej inteligencji.
Przykłady sztucznej inteligencji takie jak AlphaGo i GPT-3 wykazały zdolność do osiągania wyjątkowych wyników w dziedzinach gier planszowych i generowania tekstów.
Jednak to tylko początek.
W przyszłości możemy spodziewać się dalszego rozwoju sztucznej inteligencji, która będzie coraz bardziej zaawansowana i wszechstronna.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth, keepaspectratio]{siec_wielowarstwowa.png}
    \caption{Sieć wielowarstwowa jednokierunkowa [4]}
    \label{fig:siec}
\end{figure}
