...
K1 = 6
K2 = 4
PK_3D_K = np.zeros((len(lr_vec), K1, K2))
max_epoch = 100
PK_max = 0
lr_max_ind = 0
...
for lr_ind in range(len(lr_vec)):
    for k1 in range(1, K1+1):
        for k2 in range(1, K2+1):
            model = Model(X_train.shape[1], int(max(y) + 1), [k1, k2])
            ...
            with torch.no_grad():
                ...
                print("LR {} | K1 {} | K2 {} | PK {} ".format(lr_vec[lr_ind], k1, k2, PK))
                PK_3D_K[lr_ind, k1-1, k2-1] = PK

            if PK > PK_max:
                PK_max = PK
                lr_max_ind = lr_ind

print("Max PK: {} for LR: {} | K1: {} | K2: {}".format(PK_max, lr_vec[lr_max_ind], K1, K2))

fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
k1_vals, k2_vals = np.meshgrid(range(1, K1+1), range(1, K2+1))
ax.plot_surface(k1_vals, k2_vals, PK_3D_K[lr_max_ind].T, cmap='viridis')
ax.set_xlabel('K1')
ax.set_ylabel('K2')
ax.set_zlabel('PK')
...
for lr_ind in range(len(lr_vec)):
        for i in range(3,10,1):
            ...
            with torch.no_grad():
                ...
                correct = (torch.argmax(y_pred, dim=1) == y_test).type(torch.FloatTensor)
                PK = correct.mean().item() * 100
                print("LR {} | K1 {} | K2 {} | PK {} | layers {} ".format(lr_vec[lr_ind], K1, K2, PK, layers))
                PK_3D_K[lr_ind, K1-1, K2-1] = PK
...
PK_3D_K = PK_3D_K[:max_layers-2, :len(lr_vec)]
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
X, Y = np.meshgrid(range(3, max_layers + 1), lr_vec)
