...
Wykorzystaną funkcją kosztu w kodzie jest entropia krzyżowa.
...
Dąży się do minimalizacji funkcji kosztu, poprzez zmianę wag sieci aby osiągnąć lepsze wyniki uczenia.
Entropię krzyżową można zdefiniować następującym wzorem:
\[J(\theta)  = -\frac{1}{N} \sum_{i=1}^{N} \sum_{k=1}^{K} y_{i,k} \log(p_{i,k}) = -\frac{1}{N}\sum_{i=1}^{N} \left(y_i \log(p_i) + (1-y_i) \log(1-p_i)\right)\]
gdzie:
\begin{itemize}
    \item \(N\) -- liczba próbek w zbiorze treningowym,
    \item \(\theta\) -- parametry sieci, które chcemy uczyć,
    \item \(K\) -- liczba klas,
    \item \(y_{i,k}\) -- wartość etykiety dla próbki \(i\) i klasy \(k\) (0 lub 1),
    \item \(p_{i,k}\) -- przewidywane prawdopodobieństwo dla próbki \(i\) i klasy \(k\).
\end{itemize}
...
\begin{itemize}
    \item \(\frac{\partial J}{\partial z_i}\) to gradient funkcji kosztu względem wejścia \(z_i\) do funkcji aktywacji (np. softmax) w warstwie wyjściowej,
    \item \(p_i\) to przewidywane prawdopodobieństwo dla próbki \(i\),
    \item \(y_i\) to prawdziwa etykieta (0 lub 1) dla próbki \(i\).
\end{itemize}
